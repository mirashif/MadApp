...
export class AddressStore {
  ...
  *addAddress(addressable: UnsavedAddress) {
    if (!this.parent.auth.user || this.parent.auth.user === true) {
      throw new Error("Can't save address if auth-user is not initialized.");
    }
  ...
  get nearbyAddresses() {
    return this.all
      ...
      .sort(
        (a, b) => (a?.[DISTANCE_SYMBOL] || 0) - (b?.[DISTANCE_SYMBOL] || 0)
      );
  }
  ...
  *deleteAddress(id: string) {
    if (!this.parent.auth.user || this.parent.auth.user === true) {
      throw new Error("Auth-user not initialized, don't know what to delete.");
    }
