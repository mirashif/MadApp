...
import type { RestaurantWithAvailabilityType } from "./RestaurantStore";
...
export class BranchStore {
      ...
      .onSnapshot((snap) => {
        snap.docChanges().forEach((change) => {
          if (change.type === "added" || change.type === "modified") {
            this.upsert(change.doc.id, <BranchType>change.doc.data());
  ...
  isBranchAvailable(id: string): boolean {
    ...
    const { lockedAddress } = this.parent.lockedAddress;
